...
void GetPlayerData(int client)
{
	...
	{
		char query[255];
		FormatEx(query, sizeof(query), "SELECT * FROM %sgloves WHERE name = '%s'", g_TablePrefix, name);
...
public void T_GetPlayerDataCallback(Database database, DBResultSet results, const char[] error, int userid)
{
	...
	if(IsValidClient(clientIndex))
	{
		...
		else if (results.RowCount == 0)
		{
			...
			{
				char query[255];
				FormatEx(query, sizeof(query), "INSERT or IGNORE INTO %sgloves (name) VALUES ('%s')", g_TablePrefix, name);
				DataPack pack = new DataPack();
				pack.WriteString(name);
				pack.WriteString(query);
				db.Query(T_InsertCallback, query, pack);
				g_iGroup[clientIndex][CS_TEAM_T] = 0;
				g_iGloves[clientIndex][CS_TEAM_T] = 0;
				g_fFloatValue[clientIndex][CS_TEAM_T] = 0.0;
				g_iGroup[clientIndex][CS_TEAM_CT] = 0;
				g_iGloves[clientIndex][CS_TEAM_CT] = 0;
				g_fFloatValue[clientIndex][CS_TEAM_CT] = 0.0;
			}
		}
		else
		{
			if(results.FetchRow())
			{
				...
				{
					g_iGroup[clientIndex][j] = results.FetchInt(i);
					g_iGloves[clientIndex][j] = results.FetchInt(i + 1);
					g_fFloatValue[clientIndex][j] = results.FetchFloat(i + 2);
				}
			}
		}
	}
...
void UpdatePlayerData(int client, char[] updateFields)
{
	...
	{
		char query[1024];
		FormatEx(query, sizeof(query), "UPDATE %sgloves SET %s WHERE name = '%s'", g_TablePrefix, updateFields, name);
		DataPack pack = new DataPack();
		pack.WriteString(query);
...
public void SQLConnectCallback(Database database, const char[] error, any data)
{
	...
	else
	{
		...
		FormatEx(createQuery, sizeof(createQuery), "CREATE TABLE IF NOT EXISTS %sgloves (name varchar(128) NOT NULL PRIMARY KEY, t_group int(5) NOT NULL DEFAULT '0', t_glove int(5) NOT NULL DEFAULT '0', t_float decimal(3,2) NOT NULL DEFAULT '0.0', ct_group int(5) NOT NULL DEFAULT '0', ct_glove int(5) NOT NULL DEFAULT '0', ct_float decimal(3,2) NOT NULL DEFAULT '0.0')", g_TablePrefix);

...
public Action Team_Singularity(int client, int iArgs)
{
	...
	if (strcmp(arg, "ct") == 0)
	{
		...
		ServerCommand("bot_add_ct %s", "trace");
		ServerCommand("bot_add_ct %s", "holzt");
	...
	if (strcmp(arg, "t") == 0)
	{
		...
		ServerCommand("bot_add_t %s", "trace");
		ServerCommand("bot_add_t %s", "holzt");
...
public Action Team_Recon(int client, int iArgs)
{
	...
	if (strcmp(arg, "ct") == 0)
	{
		...
		ServerCommand("bot_add_ct %s", "Wilky");
	...
	if (strcmp(arg, "t") == 0)
	{
		...
		ServerCommand("bot_add_t %s", "Wilky");
...
public Action Team_Baecon(int client, int iArgs)
{
	...
	if (strcmp(arg, "ct") == 0)
	{
		...
		ServerCommand("bot_add_ct %s", "kyolka");
	...
	if (strcmp(arg, "t") == 0)
	{
		...
		ServerCommand("bot_add_t %s", "kyolka");
...
public Action Team_GameAgents(int client, int iArgs)
{
	...
	if (strcmp(arg, "ct") == 0)
	{
		...
		ServerCommand("bot_add_ct %s", "zrkk");
	...
	if (strcmp(arg, "t") == 0)
	{
		...
		ServerCommand("bot_add_t %s", "zrkk");
...
public Action Team_Lilmix(int client, int iArgs)
{
	...
	if (strcmp(arg, "ct") == 0)
	{
		...
		ServerCommand("bot_add_ct %s", "downie");
	...
	if (strcmp(arg, "t") == 0)
	{
		...
		ServerCommand("bot_add_t %s", "downie");
...
public Action Team_Rooster(int client, int iArgs)
{
	...
	if (strcmp(szArg, "ct") == 0)
	{
		...
		ServerCommand("bot_add_ct %s", "seaf");
	...
	if (strcmp(szArg, "t") == 0)
	{
		...
		ServerCommand("bot_add_t %s", "seaf");
...
public Action Team_Ambush(int client, int iArgs)
{
	...
	if (strcmp(szArg, "ct") == 0)
	{
		...
		ServerCommand("bot_add_ct %s", "EMZ");
...
public Action Team_Big5(int client, int iArgs)
{
	...
	{
		ServerCommand("bot_kick ct all");
		ServerCommand("bot_add_ct %s", "melkies");
	...
	{
		ServerCommand("bot_kick t all");
		ServerCommand("bot_add_t %s", "melkies");
...
public void OnFreezetimeEnd(Event eEvent, char[] szName, bool bDontBroadcast)
{
	...
	if(iCurrentRound == 0 || iCurrentRound == 15)
	{
		return;
	}
...
public Action OnPlayerRunCmd(int client, int &iButtons, int &iImpulse, float fVel[3], float fAngles[3], int &iWeapon, int &iSubtype, int &iCmdNum, int &iTickCount, int &iSeed, int iMouse[2])
{
	if (g_bFreezetimeEnd && IsValidClient(client) && IsPlayerAlive(client) && IsFakeClient(client))
	{
		...
		if (g_bIsProBot[client])
		{
			...
			/*if (!g_bBombPlanted && g_bDoExecute && (GetTotalRoundTime() - GetCurrentRoundTime() >= 60) && GetClientTeam(client) == CS_TEAM_T && !g_bHasThrownNade[client] && GetAliveTeamCount(CS_TEAM_T) >= 3 && GetAliveTeamCount(CS_TEAM_CT) > 0 && HumansOnTeam(CS_TEAM_T) >= 0 && (!IsValidClient(g_iTarget[client]) || !IsPlayerAlive(g_iTarget[client]) || g_fTargetPos[client][2] == 0))
			{

...
public Action Team_Impact(int client, int iArgs)
{
	...
	if(StrEqual(arg, "ct"))
	{
		...
		ServerCommand("bot_add_ct %s", "Switchy");
	...
	if(StrEqual(arg, "t"))
	{
		...
		ServerCommand("bot_add_t %s", "Switchy");
			...
			{				
				...
				if (g_bFreezetimeEnd && iClipAmmo > 0 && !bInReload)
				{
					if(IsValidClient(iEnt))
					{
						...
						if(!IsWeaponSlotActive(client, CS_SLOT_KNIFE) && GetEntityMoveType(client) == MOVETYPE_LADDER)
						{
							iButtons |= IN_JUMP;
				...
				if(g_bFreezetimeEnd && !g_bBombPlanted && !BotIsBusy(client) && (fHostageDistance > 100.0 || !IsValidEntity(iHostage)))
				{
					//Rifles
...
stock int GetClosestClient(int client)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsValidClient(i))
		{
			...
			else if(strcmp(szClanTag, "Chaos") == 0) //28th
			{
				if (!IsTargetInSightRange(client, i, 70.0))
					continue;	
			}
			else if(strcmp(szClanTag, "Nemiga") == 0) //27th
			{
				if (!IsTargetInSightRange(client, i, 80.0))
					continue;	
			}
			else if(strcmp(szClanTag, "VP") == 0) //26th
			{
			...
			else if(strcmp(szClanTag, "Astralis") == 0) //17th
			{
			...
			else if(strcmp(szClanTag, "NiP") == 0) //13th
			{
			...
			else if(strcmp(szClanTag, "Heroic") == 0) //11th
			{
			...
			else if(strcmp(szClanTag, "fnatic") == 0) //8th
			{
				if (!IsTargetInSightRange(client, i, 270.0))
					continue;	
			}
			else if(strcmp(szClanTag, "coL") == 0) //7th
			{
...
public void Pro_Players(char[] szBotName, int client)
{
	...
	//Lyngby Players
	if((StrEqual(szBotName, "birdfromsky")) || (StrEqual(szBotName, "Twinx")) || (StrEqual(szBotName, "FeTiSh")) || (StrEqual(szBotName, "Raalz")) || (StrEqual(szBotName, "Cabbi")))
	{
	...
	//Singularity Players
	if((StrEqual(szBotName, "nicoodoz")) || (StrEqual(szBotName, "Casle")) || (StrEqual(szBotName, "Remoy")) || (StrEqual(szBotName, "TOBIZ")) || (StrEqual(szBotName, "Celrate")))
	{
	...
	//Impact Players
	if((StrEqual(szBotName, "DaneJoris")) || (StrEqual(szBotName, "JoJo")) || (StrEqual(szBotName, "tconnors")) || (StrEqual(szBotName, "Switchy")) || (StrEqual(szBotName, "insane")))
	{
